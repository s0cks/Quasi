...
class MockBytecodeGenerator{
    ...
    @SuppressWarnings("unchecked")
    public <T> Class<T> get(MockSpec spec){
        this.lock.lock();
        try{
    ...
    private static final class ByteArrayClassLoader
    extends ClassLoader{
    ...
    private static final class CachedBytecodeGenerator{
        ...
        private <T> Class<? extends T> generate(MockSpec spec){
            ...
            cw.visit(Opcodes.V1_7, Opcodes.ACC_PUBLIC + Opcodes.ACC_SUPER + Opcodes.ACC_FINAL, name, null, "java/lang/Object", new String[]{typeName(spec.mockKey.rawType)});
            ...
            for(Method m : spec.mockKey.rawType.getDeclaredMethods()){
                if(Modifier.isPublic(m.getModifiers())){
                    if(!spec.getActionMap().containsKey(m.getName())){
                        ...
                        spec.getActionMap().get(m.getName()).generateBytecode(cw, m, mv, spec, name);
                    }
                }
            }
            ...
            mv.visitEnd();
            cw.visitEnd();
            return (Class<? extends T>) ByteArrayClassLoader.instance().load(name, cw.toByteArray());
        }
